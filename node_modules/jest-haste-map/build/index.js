...
function _jestWorker() {
  const data = import('jest-worker');
  _jestWorker = function () {
    return data;
  };
  return data;
}
var _HasteFS = _interopRequireDefault(import('./HasteFS'));
var _ModuleMap = _interopRequireDefault(import('./ModuleMap'));
var _constants = _interopRequireDefault(import('./constants'));
var _node = import('./crawlers/node');
var _watchman = import('./crawlers/watchman');
var _getMockName = _interopRequireDefault(import('./getMockName'));
var fastPath = _interopRequireWildcard(import('./lib/fast_path'));
var _getPlatformExtension = _interopRequireDefault(
  import('./lib/getPlatformExtension')
);
var _isWatchmanInstalled = _interopRequireDefault(
  import('./lib/isWatchmanInstalled')
);
var _normalizePathSep = _interopRequireDefault(
  import('./lib/normalizePathSep')
);
var _FSEventsWatcher = import('./watchers/FSEventsWatcher');
var _NodeWatcher = _interopRequireDefault(import('./watchers/NodeWatcher'));
var _WatchmanWatcher = _interopRequireDefault(
  import('./watchers/WatchmanWatcher')
);
var _worker = import('./worker');
...
// understand `require`.
const {version: VERSION} = import('../package.json');
...
class HasteMap extends _events().EventEmitter {
  ...
  static getStatic(config) {
    if (config.haste.hasteMapModulePath) {
      return import(config.haste.hasteMapModulePath);
    }
    return HasteMap;
  }
  static async create(options) {
    if (options.hasteMapModulePath) {
      const CustomHasteMap = import(options.hasteMapModulePath);
  ...
  async setupCachePath(options) {
    ...
    if (options.hasteImplModulePath) {
      const hasteImpl = import(options.hasteImplModulePath);
