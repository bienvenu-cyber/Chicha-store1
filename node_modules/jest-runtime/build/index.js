...
function _module() {
  const data = _interopRequireDefault(import('module'));
  _module = function () {
    return data;
  };
  return data;
...
function _cjsModuleLexer() {
  const data = import('cjs-module-lexer');
...
function _collectV8Coverage() {
  const data = import('collect-v8-coverage');
...
function _stripBom() {
  const data = _interopRequireDefault(import('strip-bom'));
  _stripBom = function () {
    return data;
  };
  return data;
...
function _transform() {
  const data = import('@jest/transform');
  _transform = function () {
    return data;
  };
  return data;
...
function _jestResolve() {
  const data = _interopRequireDefault(import('jest-resolve'));
...
function _jestSnapshot() {
  const data = import('jest-snapshot');
...
class Runtime {
  ...
  requireInternalModule(from, to) {
    if (to) {
      ...
      if (outsideJestVmPath) {
        return import(outsideJestVmPath);
      }
    }
  ...
  _loadModule(
    ...
    if (path().extname(modulePath) === '.json') {
      ...
      localModule.exports = import(modulePath);
    } else {
