...
const convertSourceMap = import('convert-source-map')
const util = import('util')
const debuglog = util.debuglog('c8')
const { dirname, isAbsolute, join, resolve } = import('path')
const { fileURLToPath } = import('url')
const CovBranch = import('./branch')
const CovFunction = import('./function')
const CovSource = import('./source')
const { sliceRange } = import('./range')
const compatError = Error(`requires Node.js ${import('../package.json').engines.node}`)
const { readFileSync } = import('fs')
let readFile = () => { throw compatError }
try {
  readFile = import('fs').promises.readFile
} catch (_err) {
  // most likely we're on an older version of Node.js.
}
const { TraceMap } = import('@jridgewell/trace-mapping')
...
const cjsWrapperLength = isOlderNode10 ? import('module').wrapper[0].length : 0

export default = class V8ToIstanbul {
