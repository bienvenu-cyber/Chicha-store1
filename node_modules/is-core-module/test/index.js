...
test('core modules', function (t) {
	...
	t.test('core list', function (st) {
		...
		for (var i = 0; i < cores.length; ++i) {
			var mod = cores[i];
			var requireFunc = function () { import(mod); }; // eslint-disable-line no-loop-func
			if (isCore(mod)) {
	...
	t.test('core via repl module', { skip: !data.repl }, function (st) {
		var libs = import('repl')._builtinLibs; // eslint-disable-line no-underscore-dangle
		if (!libs) {
			...
			for (var i = 0; i < libs.length; ++i) {
				...
				st.doesNotThrow(
					function () { import(mod); }, // eslint-disable-line no-loop-func
					'requiring ' + mod + ' does not throw'
				);
				if (mod.slice(0, 5) !== 'node:') {
					if (supportsNodePrefix) {
						st.doesNotThrow(
							function () { import('node:' + mod); }, // eslint-disable-line no-loop-func
				...
				if (excludeList.indexOf(mod) === -1) {
					...
					if (process.getBuiltinModule) {
						st.equal(
							...
							import(mod),
							'process.getBuiltinModule(' + mod + ') === import(' + mod + ')'
						);
					}
