...
var path = _interopRequireWildcard(import('path'));
var _util = import('util');
var fs = _interopRequireWildcard(import('graceful-fs'));
var _semver = _interopRequireDefault(import('semver'));
...
import(require.resolve('@babel/generator', {
...
const {
  ...
  traverse,
  traverseFast
} = import(require.resolve('@babel/types', { // @ts-expect-error requireOutside Babel transform
  [(globalThis['jest-symbol-do-not-touch'] || globalThis.Symbol).for(
    'jest-resolve-outside-vm-option'
  )]: true
}));
// @ts-expect-error requireOutside Babel transform
const {parseSync} = import(require.resolve('@babel/core', {
  [(globalThis['jest-symbol-do-not-touch'] || globalThis.Symbol).for(
    'jest-resolve-outside-vm-option'
  )]: true
}));
function saveInlineSnapshots(snapshots, rootDir, prettierPath) {
  ...
  if (prettierPath) {
    try {
      // @ts-expect-error requireOutside Babel transform
      prettier = import(require.resolve(prettierPath, {
        [(globalThis['jest-symbol-do-not-touch'] || globalThis.Symbol).for(
          'jest-resolve-outside-vm-option'
        )]: true
      }));
